...
struct Circle {
	...
	Circle() : center(Point(-1, -1)), r(-1) {}
	Circle(const Point &p, double r) : center(p), r(r) {}

	bool contains(const Point &p) const {
		return dist(center, p) <= r + EPSILON;
	}
...
Circle makeDiameter(const Point &a, const Point &b) {
	return Circle(Point((a.x + b.x) / 2.0, (a.y + b.y) / 2.0), dist(a, b) / 2.0);
}
...
Circle makeCircleTwoPoints(vector<Point> points, const Point &p, const Point &q) {
	...
	for (int i = 0, sz = points.size(); i < sz; ++i) {
		Point r = points[i];
		Point pq = toVec(q, p);
		double cross_value = cross(pq, toVec(r, p));
		Circle c = makeCircumcircle(p, q, r);
		if (c.r < 0)
			continue;
		else if (cross_value > 0 && (left.r < 0 || cross(pq, toVec(c.center, p)) > cross(pq, toVec(left.center, p))))
			left = c;
		else if (cross_value < 0 && (right.r < 0 || cross(pq, toVec(c.center, p)) < cross(pq, toVec(right.center, p))))
			right = c;
	}
